  ...
  "GET /posts" should {
    "何も投稿しない場合は空の配列を返す" in {
      WsTestClient.withClient { ws =>
        val response = Await.result(ws.url(s"http://localhost:$port/api/posts").get(), Duration.Inf)
  ...
  "POST /posts" should {
    "投稿したものが返される" in {
      WsTestClient.withClient { ws =>
        val body = "test post"
        val postResponse =
          Await.result(ws.url(s"http://localhost:$port/api/posts").post(Map("post" -> Seq(body))), Duration.Inf)
        assert(postResponse.status === 200)
        val getResponse = Await.result(ws.url(s"http://localhost:$port/api/posts").get(), Duration.Inf)
